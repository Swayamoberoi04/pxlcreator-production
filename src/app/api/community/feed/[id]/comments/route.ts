/**
 * GET   /api/community/feed/[id]/comments — list comments (nested), newest post first
 * POST  /api/community/feed/[id]/comments — create a comment
 *
 * Identical logic to the channel-post comments route, minus the channel
 * membership/scoping — a feed post has no channel to check.
 *
 * GET query params: ?page=1 ?limit=50
 * POST body: { body (max 2000 chars), parent_id? }
 * Auth: GET public; POST requires auth.
 */
...
export async function GET(req: NextRequest, { params }: Params) {
  ...
  try {
    ...
    const profileMap = new Map((authorProfiles ?? []).map((p: { firebase_uid: string }) => [p.firebase_uid, p]))
    ...
    for (const comment of comments as Array<Record<string, unknown>>) {
      const enriched = { ...comment, author: profileMap.get(comment.author_uid as string) ?? null, replies: [] as Array<Record<string, unknown>> }
    ...
    for (const comment of topLevel) comment.replies = replyMap.get(comment.id as string) ?? []
...
export async function POST(req: NextRequest, { params }: Params) {
  ...
  try {
    ...
    const { data: post, error: postError } = await supabase
      .from("channel_posts")
      .select("id, author_uid, comment_count, title, is_removed")
      .eq("id", postId)
      .maybeSingle()
    ...
    const { data: comment, error: insertError } = await supabase
      .from("post_comments")
      .insert({ post_id: postId, author_uid: uid, parent_id: parent_id ?? null, body: commentBody, like_count: 0 })
      .select("*")
      .single()
    if (insertError) {

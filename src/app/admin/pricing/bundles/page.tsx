    ...
    <div className="flex flex-col gap-8 p-8 max-w-4xl">
      ...
      <div className="rounded-xl border border-amber-500/20 bg-amber-500/[0.04] px-5 py-4 flex gap-3">
        ...
        <div>
          ...
          <p className="text-[0.8125rem] text-white/70 mt-0.5 leading-relaxed">
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center justify-between gap-4 px-6 py-4 border-b border-white/[0.05]">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className="text-[0.9375rem] font-bold text-white/92 truncate">{bundle.name}</p>
                      <p className="text-[0.75rem] text-white/70 truncate mt-0.5">{bundle.tagline}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 flex-shrink-0">
                    ...
                    <Link
                      ...
                      target="_blank"
                      className="text-[0.75rem] text-white/70 hover:text-gold transition-colors px-3 py-1.5 rounded-lg border border-white/10 hover:border-gold/30"
                    >
                      View ↗
                    </Link>
                  </div>
                ...
                <div className="px-6 py-4">
                  <p className="text-[0.65rem] font-bold tracking-widest uppercase text-white/70 mb-3">
                    Included Packs
                  </p>
                          ...
                          <div className="min-w-0">
                            {pack.slug ? (
                              <Link
                                ...
                                target="_blank"
                                className="text-[0.8125rem] text-white/85 hover:text-gold transition-colors truncate block"
                              >
                                {pack.name} ↗
                              </Link>
                            ) : (
                            ...
                            <p className="text-[0.7rem] text-white/70">{pack.presetCount} presets · {pack.category}</p>
                          </div>
                        </div>
                        <div className="text-right flex-shrink-0">
                          {pack.priceUsd !== undefined ? (
                            <span className={cn(
                              ...
                              pack.priceUsd === 0 ? "text-emerald-400/60" : "text-white/85"
                            )}>
                              {pack.priceUsd === 0 ? "Free" : `$${pack.priceUsd.toFixed(2)}`}
                            </span>
                          ) : (
                            <span className="text-[0.75rem] text-white/70">— no price</span>
                          )}
                        </div>
                      </div>
                    ))}
                ...
                <div className="border-t border-white/[0.05] px-6 py-4 grid grid-cols-3 gap-4">
                  <div>
                    <p className="text-[0.65rem] text-white/70 uppercase tracking-wide mb-0.5">Individual Value</p>
                    <p className="text-[1rem] font-bold text-white/70 line-through">
                      ${bundle.individualValueUsd.toFixed(2)}
                    </p>
                  </div>
                  <div>
                    <p className="text-[0.65rem] text-white/70 uppercase tracking-wide mb-0.5">Bundle Price</p>
                    <p className="text-[1rem] font-bold text-gold">
                      ${bundle.price.toFixed(2)}
                    </p>
                  </div>
                  <div>
                    <p className="text-[0.65rem] text-white/70 uppercase tracking-wide mb-0.5">Customer Saves</p>

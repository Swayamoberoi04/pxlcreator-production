      ...
      {nextTask ? (
        <div className="p-5 flex flex-col gap-3">
          ...
          <div className="flex items-center gap-3">
            <span className="text-xs text-muted/40">⏱ {nextTask.estimatedMinutes} min</span>
            <button
              type="button"
    ...
    <div className="rounded-2xl border border-border bg-surface overflow-hidden">
      ...
      {todayData && (
        <div className="p-5 flex flex-col gap-3">
          ...
          <div className="flex items-center gap-3">
            <span className="text-xs text-muted/40">⏱ {todayData.estimatedMinutes} min</span>
            <button
              type="button"
    ...
    <div className="rounded-2xl border border-border bg-surface overflow-hidden">
      ...
      {next ? (
        <div className="p-5 flex flex-col gap-3">
          ...
          <div className="flex items-center gap-3">
            <span className="text-xs text-muted/40">⏱ {next.duration}</span>
            <button
              type="button"
              onClick={handleComplete}
              disabled={marking || done}
...
export default function DashboardPage() {
  ...
  const accentColor = dna?.primaryColor ?? "#ffd700"
      ...
      <AnimatePresence mode="wait">
        <motion.div key="dashboard-content" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.4 }}
          ...
          <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.34, duration: 0.5 }}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                { icon: "◈", label: "Presets For You",   sub: "Sorted by your style",    href: "/store/for-you",  color: "#ffd700" },

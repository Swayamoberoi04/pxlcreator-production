...
export function CheckoutClient() {
  ...
  /* â”€â”€ Coupon: validate code against current subtotal â”€â”€ */
  ...
  /* â”€â”€ Main Razorpay payment flow â”€â”€ */
  const handlePayment = useCallback(async () => {
    ...
    try {
      ...
      const rzp = new window.Razorpay({
        ...
        modal: {
          ondismiss: () => {
            /* User closed modal â€” go back to form */
            setStep("form")
          },
        },
      })
  ...
  /* â”€â”€ Processing / verifying overlay â”€â”€ */
  if (step === "processing" || step === "verifying") {
    const msg = step === "verifying" ? "Verifying paymentâ€¦" : "Preparing secure checkoutâ€¦"
    return (
      <div className="relative w-full bg-background overflow-hidden min-h-[70vh] flex items-center justify-center">
        ...
        <Container size="wide" className="relative z-10 py-10 sm:py-14">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-8 xl:gap-12 items-start">
                  ...
                  <div className="flex items-center justify-between rounded-xl bg-emerald-500/10 border border-emerald-500/20 px-4 py-3">
                    <div>
                      ...
                      <p className="text-xs text-muted mt-0.5">
                        You save â‚¹{couponResult.discount_amount_inr.toLocaleString("en-IN")}
                      </p>
                    </div>
                    <button
                      type="button"
            ...
            <div className="flex flex-col gap-4 lg:sticky lg:top-24">
                  ...
                  {couponResult && (
                    <div className="flex justify-between text-small">
                      ...
                      <span className="text-emerald-400 font-medium">
                        âˆ’â‚¹{couponResult.discount_amount_inr.toLocaleString("en-IN")}
                      </span>
                    </div>
                  )}
                  <div className="flex justify-between text-small">
                  ...
                  <div className="flex justify-between border-t border-border pt-2.5 mt-1">
                    ...
                    <div className="text-right">
                      <div className="font-display font-bold text-xl text-gold">
                        â‚¹{(couponResult ? couponResult.final_total_inr : toInr(total)).toLocaleString("en-IN")}
                      </div>
                      {couponResult && (
                        <div className="text-xs text-muted line-through">
                          â‚¹{toInr(total).toLocaleString("en-IN")}
                        </div>
                      )}
                    </div>
                  </div>
              ...
              >
                <LockIcon size={16} />
                Pay â‚¹{(couponResult ? couponResult.final_total_inr : toInr(total)).toLocaleString("en-IN")} securely
              </button>

              {!scriptReady && (
                <p className="text-center text-xs text-muted animate-pulse">Loading payment SDKâ€¦</p>
              )}
